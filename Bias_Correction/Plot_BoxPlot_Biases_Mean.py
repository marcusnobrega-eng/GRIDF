# 01_boxplot_bias_station_MEAN_ONLY.py
# ---------------------------------------------------------
# One bias per station using AVERAGES:
...
OUT_PNG = r'G:\My Drive\multi_product_bias_plots\boxplot_bias_station_means.png'

# Font
FONT_PATH_MONTSERRAT = r'C:/Users/marcu/PycharmProjects/Sucept_Maps/Montserrat/Montserrat-Regular.ttf'
FONT_SIZE = 9

# Screening (applied to event pairs before station means)
...
# Y-axis display range for bias factors
Y_MIN, Y_MAX = 0.0, 2.0
...
def load_station_mean_biases(pairs_dir, pairs_glob, zeta_csv, precip_col_hint=None):
    ...
    # Discard stations with tiny means to avoid blow-ups
    grp = grp[(grp['Gmean'] >= MIN_MM) & (grp['Pmean'] >= MIN_MM)]
    if grp.empty:
        return None, None

    # Station-level biases from means
    raw_bias = (grp['Pmean'].to_numpy() / grp['Gmean'].to_numpy())
    cor_bias = (grp['Pmean'].to_numpy() * grp['zeta'].to_numpy() / grp['Gmean'].to_numpy())
...
def main():
    ...
    for cfg in CONFIGS:
        raw, cor = load_station_mean_biases(
    ...
    ax.set_ylabel("Bias factor from station means")

    # Reference line (unbiased)
