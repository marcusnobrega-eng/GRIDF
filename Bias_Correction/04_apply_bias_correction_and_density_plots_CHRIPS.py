# 04_apply_bias_correction_and_density_plots.py
# ---------------------------------------------------------
# Two side-by-side scatter-density panels with Turbo colormap.
# Left  : RAW   (x = Rain Gauge, y = CHIRPS)
# Right : CORR  (x = Rain Gauge, y = zeta * CHIRPS)
...
def main():
    ...
    # QC
    need = {'station_id','pr_g','chirps_mm'}
    ...
    pairs = pairs[(pairs['pr_g'] >= MIN_MM) & (pairs['chirps_mm'] >= MIN_MM)]
    ...
    df['chirps_corr'] = df['chirps_mm'] * df['zeta']

    # Arrays for plotting
    x_g   = df['pr_g'].to_numpy()
    y_raw = df['chirps_mm'].to_numpy()
    y_cor = df['chirps_corr'].to_numpy()
    ...
    panel_scatter_density(axs[0], x_g, y_raw, lim, norm,
                          title="Raw", fit_color='tab:gray')
    panel_scatter_density(axs[1], x_g, y_cor, lim, norm,
                          title="Bias-corrected", fit_color='tab:blue')
