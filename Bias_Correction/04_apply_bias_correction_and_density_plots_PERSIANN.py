...
# Left  : RAW   (x = Rain Gauge, y = PERSIANN)
# Right : CORR  (x = Rain Gauge, y = zeta * PERSIANN)
...
OUT_PNG    = r'G:\My Drive\persiann_bias_pairs\density_raw_vs_corrected_turbo_persiann.png'
...
def detect_precip_col(df: pd.DataFrame) -> str:
    """Prefer PERSIANN ('persiann_mm'); fallback to CHIRPS ('chirps_mm')."""
    if 'persiann_mm' in df.columns: return 'persiann_mm'
    if 'chirps_mm'   in df.columns: return 'chirps_mm'
    raise ValueError("Could not find 'persiann_mm' or 'chirps_mm' in pairs dataframe.")
...
def main():
    ...
    # Detect precip column (PERSIANN or CHIRPS fallback)
    precip_col = detect_precip_col(pairs)
    product_name = "PERSIANN" if precip_col == "persiann_mm" else "CHIRPS"
