# 01_extract_extreme_pairs_ANA_PERSIANN_GEE.py
# ---------------------------------------------------------
# Purpose:
#   From a wide ANA CSV (one row per station, daily columns like 1/1/1994),
#   find per-year >P95 gauge extremes with declustering, sample PERSIANN-CDR at those
#   dates/points in Google Earth Engine, and export one CSV per year to Drive:
#     station_id, date, persiann_mm, pr_g, ratio
#
# Requirements:
...
#
# Notes:
#   - This submits one GEE export task per year.
#   - PERSIANN-CDR coverage is 60°S–60°N at 0.25° resolution; daily mm/day.
#   - Dataset has ~quarterly latency.
# ---------------------------------------------------------
...
OUT_GDRIVE_FOLDER = "persiann_bias_pairs"  # renamed
SAMPLE_SCALE_M = 27830  # ~0.25° native PERSIANN-CDR resolution
...
def main():
    ...
    # Docs: NOAA/PERSIANN-CDR (0.25°, 60S–60N)
    ...
    for y in years:
        ...
        for dstr, entry in tqdm(per_date.items(), desc=f"EE sampling {y}"):
            ...
            # PERSIANN image for that date (daily)
            d0 = ee.Date(dstr)
            img0 = PERSIANN.filterDate(d0, d0.advance(1, 'day')).first()
            img = ee.Image(
                ee.Algorithms.If(
                    img0,
                    ee.Image(img0).select('precipitation'),  # PERSIANN-CDR band name
                    ee.Image.constant(0).rename('precipitation').toFloat()
                )
            )

            # Sample at stations (native 0.25° ~ 27.8 km)
            ...
            samples2 = (
                ...
                .filter(ee.Filter.gt('persiann_mm', 1))
        ...
        task = ee.batch.Export.table.toDrive(
            collection=fc_year,
            description=f"PERSIANN_BiasPairs_{y}",
            folder=OUT_GDRIVE_FOLDER,
            fileNamePrefix=f"persiann_bias_pairs_{y}",
            fileFormat='CSV',
            selectors=['station_id', 'lat', 'lon', 'date', 'persiann_mm', 'pr_g', 'ratio']
        )
        task.start()
