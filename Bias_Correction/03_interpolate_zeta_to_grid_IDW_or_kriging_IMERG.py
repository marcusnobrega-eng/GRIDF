...
#
# Output:
#   - GeoTIFF bias map (EPSG:4326), e.g., zeta_map_010deg.tif
#
# Reqs:
...
# ------------- USER INPUTS -------------
# IMERG output from step 02
ZETA_CSV      = r'G:/My Drive/imerg_bias_pairs/zeta_per_station.csv'  # <- IMERG ζ table
...
OUT_TIF       = r'G:/My Drive/imerg_bias_pairs/zeta_map_010deg.tif'   # <- IMERG map

GRID_RES_DEG  = 0.10     # ~IMERG native resolution (0.1°)
...
IDW_POWER     = 2.0
IDW_K         = 8
IDW_CAP_RANGE = (0.25, 10)  # clamp final raster (keep same as before unless you prefer different)
...
def main():
    ...
    if shapes is not None:
        mask_outside = geometry_mask(shapes, z_grid.shape, transform, invert=False, all_touched=False)
