...
# ---------------------------------------------------------
# Purpose:
#   Read yearly "persiann_bias_pairs_YYYY.csv" files produced by Phase A,
#   compute a robust station-level bias factor ζ from event-wise ratios
#   (default: median of ratio = pr_g / persiann_mm), and save:
...
IN_DIR  = r'G:\My Drive\persiann_bias_pairs'     # <- switched folder name
IN_GLOB = "persiann_bias_pairs_*.csv"            # <- switched file prefix
...
# Output
OUT_CSV = r'G:\My Drive\persiann_bias_pairs\zeta_per_station.csv'  # <- switched output path
...
#   'slope0' -> regression slope through origin of pr_g ~ zeta * precip_mm
ZETA_METHOD = 'mean'
# ---------------------------------------------------------
...
def detect_precip_col(df: pd.DataFrame) -> str:
    ...
    Prefers PERSIANN ('persiann_mm'); falls back to CHIRPS ('chirps_mm').
    """
    if 'persiann_mm' in df.columns:
        return 'persiann_mm'
    if 'chirps_mm' in df.columns:
        return 'chirps_mm'
    raise ValueError("Neither 'persiann_mm' nor 'chirps_mm' found in input data.")


def zeta_from_pairs(df_station: pd.DataFrame, method: str = 'median', precip_col: str = 'persiann_mm') -> float:
    """
...
def main():
    ...
    for f in files:
        ...
        # Determine which precip column we have (PERSIANN or CHIRPS fallback)
        ...
        if 'pr_g' not in df.columns:
            raise ValueError(f"'pr_g' not found in {f}")
