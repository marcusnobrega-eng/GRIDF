...
# ---------------------------------------------------------
# Purpose:
#   Read yearly "xavier_bias_pairs_YYYY.csv" files produced by Phase A,
#   compute a robust station-level bias factor ζ from event-wise ratios
#   (default: median of pr_g / xavier_pr_mm), and save:
...
IN_DIR  = r'G:\My Drive\xavier_bias_pairs'     # <- Xavier folder
IN_GLOB = "xavier_bias_pairs_*.csv"            # <- Xavier file prefix
...
# Output
OUT_CSV = r'G:\My Drive\xavier_bias_pairs\zeta_per_station.csv'  # <- Xavier output path
...
def detect_precip_col(df: pd.DataFrame) -> str:
    ...
    Priority: Xavier ('xavier_pr_mm'), then IMERG ('imerg_mm'),
              then PERSIANN ('persiann_mm'), then CHIRPS ('chirps_mm').
    """
    if 'xavier_pr_mm' in df.columns:
        return 'xavier_pr_mm'
    ...
    if 'chirps_mm' in df.columns:
        return 'chirps_mm'
    raise ValueError(
        "No precip column found (expected one of 'xavier_pr_mm', 'imerg_mm', 'persiann_mm', 'chirps_mm')."
    )


def zeta_from_pairs(df_station: pd.DataFrame, method: str = 'median', precip_col: str = 'xavier_pr_mm') -> float:
    """
    Compute station-level ζ from event pairs.
    Expected columns: ['ratio', 'pr_g', precip_col] (and 'date', 'station_id').

    ζ interprets as the multiplicative bias to apply to the gridded product:
...
def main():
    ...
    for f in files:
        ...
        # Determine which precip column we have (Xavier preferred)
