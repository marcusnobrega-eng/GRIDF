...
# Left  : RAW   (x = Rain Gauge, y = IMERG)
# Right : CORR  (x = Rain Gauge, y = zeta * IMERG)
...
OUT_PNG    = r'G:\My Drive\imerg_bias_pairs\density_raw_vs_corrected_turbo_imerg.png'
...
def detect_precip_col(df: pd.DataFrame) -> str:
    """Prefer IMERG ('imerg_mm'); fallback to PERSIANN ('persiann_mm') or CHIRPS ('chirps_mm')."""
    ...
    raise ValueError("Could not find 'imerg_mm', 'persiann_mm', or 'chirps_mm' in pairs dataframe.")

def product_label_from_col(col: str) -> str:
    return {'imerg_mm':'IMERG', 'persiann_mm':'PERSIANN', 'chirps_mm':'CHIRPS'}.get(col, col)
...
def main():
    ...
    # Detect precip column (IMERG preferred)
