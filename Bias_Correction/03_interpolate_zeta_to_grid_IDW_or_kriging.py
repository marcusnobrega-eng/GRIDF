...
#
# Output:
#   - GeoTIFF bias map (EPSG:4326), e.g., zeta_map_005deg.tif
#
# Reqs:
...
# ------------- USER INPUTS -------------
ZETA_CSV      = 'G:/My Drive/chirps_bias_pairs/zeta_per_station.csv'       # station_id,zeta,lat,lon[,n_pairs]
BOUNDARY_PATH = 'C:/Users/marcu/OneDrive - University of Arizona/Desktop/Desktop_Folder/ANA_Analysis/Rainfall_Distribution/Brasil_Shapefile/brazil_Brazil_Country_Boundary.shp'        # optional; .shp/.geojson; set "" to skip
OUT_TIF       = 'G:/My Drive/chirps_bias_pairs/zeta_map_005deg.tif'

GRID_RES_DEG  = 0.05     # ~CHIRPS resolution
...
IDW_POWER     = 2.0
IDW_K         = 8        # neighbors
IDW_CAP_RANGE = (0.25, 10)  # clamp final raster
...
# Filters
MIN_PAIRS     = 10       # drop stations with < MIN_PAIRS (if column exists)
ZETA_CLIP     = (0.05, 10.0)   # drop crazy zeta values before interp
# ---------------------------------------
...
def _ensure_proj_env():
    ...
    if cur and os.path.exists(os.path.join(cur, "proj.db")):
        return
    # Common bundled locations inside rasterio wheel
...
def load_boundary_shapes(path: str):
    """Return (shapes, bounds) where shapes is a list of GeoJSON-like
    geometries in EPSG:4326 and bounds=(minx,miny,maxx,maxy). Return (None,None) if no path."""
    if not path:
        return None, None
    if not os.path.exists(path):
        raise FileNotFoundError(path)
    # Lazy import geopandas + shapely only if needed
    try:
...
def save_geotiff(path, array2d, transform, nodata=np.nan, crs="EPSG:4326"):
    ...
    _ensure_proj_env()  # <-- NEW: ensure PROJ_LIB points to proj.db
    ...
    try:
        ...
        # Fallback: write without CRS (you can assign it later in GIS/QGIS)
        profile_no_crs = dict(profile)
...
def main():
    ...
    if shapes is not None:
        # Use the boundary’s envelope (covers the whole country)
        pass  # 'bounds' already set by load_boundary_shapes
    else:
        # Fall back to points’ bbox
    ...
    # geometry_mask(geometries, out_shape, transform, invert=False, all_touched=False)
    # invert=False  -> mask == True OUTSIDE the shapes
    # We want values INSIDE Brazil, so keep ~mask_outside
