...
# ---------------------------------------------------------
# Multiply each annual-maximum IMERG GeoTIFF by the bias factor (zeta) map.
# Inputs  : G:\My Drive\IMERG_Max\IMERG_MaxDaily_0p1deg_YYYY_Brazil.tif (e.g., 2020–2024)
# Zeta    : single GeoTIFF with zeta(x,y) for Brazil (time-invariant; IMERG-derived)
# Output  : G:\My Drive\IMERG_Max\IMERG_BiasCorrected_AMaxDaily\IMERG_MaxDaily_0p1deg_YYYY_Brazil_BC.tif
#
...
# ---------- USER SETTINGS ----------
IN_DIR    = r"G:\My Drive\IMERG_Max"
IN_GLOB   = r"IMERG_MaxDaily_0p1deg_\d{4}_Brazil\.tif"   # regex pattern for file names
ZETA_TIF  = r"G:\My Drive\imerg_bias_pairs\zeta_map_010deg.tif"  # IMERG ζ raster
OUT_DIR   = r"G:\My Drive\IMERG_Max\IMERG_BiasCorrected_AMaxDaily"

YEAR_MIN, YEAR_MAX = 2000, 2024
CAP_ZETA  = (0.25, 5.0)     # set to None to disable, or adjust range
RESAMPLE  = Resampling.bilinear  # resampling for zeta onto IMERG grid
...
def list_inputs():
    """Return sorted list of (path, year) matching IN_GLOB (regex) and year range."""
    out = []
    for f in os.listdir(IN_DIR):
        if re.match(IN_GLOB, f):
            m = re.search(r"_(\d{4})_", f)
            if m:
                y = int(m.group(1))
...
def main():
    ...
    inputs = list_inputs()
    if not inputs:
        raise SystemExit(f"No input files found in {IN_DIR} matching regex {IN_GLOB}")
