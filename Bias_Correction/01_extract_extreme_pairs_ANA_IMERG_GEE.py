# 01_extract_extreme_pairs_ANA_IMERG_GEE.py
# ---------------------------------------------------------
# Purpose:
#   From a wide ANA CSV (one row per station, daily columns like 1/1/1994),
#   find per-year >P95 gauge extremes with declustering, sample IMERG (daily)
#   at those dates/points in Google Earth Engine, and export one CSV per year:
#     station_id, date, imerg_mm, pr_g, ratio
#
# Requirements:
...
#   - IMERG daily asset used here:
#       'projects/climate-engine-pro/assets/ce-gpm-imerg-daily'
#     with band 'precipitationCal' [mm/day], ~0.1° (~11.1 km).
# ---------------------------------------------------------
...
OUT_GDRIVE_FOLDER = "imerg_bias_pairs"
SAMPLE_SCALE_M = 11132  # ~0.1° native IMERG resolution (approx. 11.1 km at equator)
...
def main():
    ...
    # ====== IMERG daily (mm/day) ======
    # Climate Engine daily IMERG asset with band 'precipitationCal' (mm/day)
    IMERG = ee.ImageCollection('projects/climate-engine-pro/assets/ce-gpm-imerg-daily')

    # ---------- Discover schema / years ----------
    meta_cols, all_date_cols, file_y0, file_y1 = identify_columns(CSV_PATH)

    # Use requested IMERG window (2020–2024), but clip to file’s available range
    ...
    for y in years:
        ...
        for dstr, entry in tqdm(per_date.items(), desc=f"EE sampling {y}"):
            ...
            # IMERG daily image for that date (mm/day)
            d0 = ee.Date(dstr)
            img0 = IMERG.filterDate(d0, d0.advance(1, 'day')).first()
            img = ee.Image(
                ee.Algorithms.If(
                    img0,
                    ee.Image(img0).select('precipitationCal'),  # IMERG daily band
                    ee.Image.constant(0).rename('precipitationCal').toFloat()
                )
            )

            # Sample at stations (native ~0.1°)
            samples = ee.Image(img).sampleRegions(
            ...
            def with_props(feat):
                ...
                prc = ee.Number(feat.get('precipitationCal'))
            ...
            samples2 = (
                ...
                .filter(ee.Filter.gt('imerg_mm', 1))
        ...
        task = ee.batch.Export.table.toDrive(
            collection=fc_year,
            description=f"IMERG_BiasPairs_{y}",
            folder=OUT_GDRIVE_FOLDER,
            fileNamePrefix=f"imerg_bias_pairs_{y}",
            fileFormat='CSV',
            selectors=['station_id', 'lat', 'lon', 'date', 'imerg_mm', 'pr_g', 'ratio']
        )
        task.start()
